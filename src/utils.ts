...
export const calculatePeriodYears = (baseDate: Date, endDate: Date, mode: string = 'ceil'): number => {
    ...
    if (mode === 'calendar_months') {
        const months = (endDate.getFullYear() - baseDate.getFullYear()) * 12 + (endDate.getMonth() - baseDate.getMonth());
    ...
    if (mode === 'ceil') {
        ...
        const targetYear = baseDate.getFullYear() + Math.floor((baseDate.getMonth() + months) / 12);
        const targetMonth = (baseDate.getMonth() + months) % 12;
...
export const processRow = (
    ...
    try {
        ...
        // --- 初期ポイント計算 (ここだけは pointCalcBaseDate を使う: ポイント付与ルールに準拠) ---
        ...
        if (inputLos !== undefined) {
            ...
            // Calculate point tenure using pointCalcBaseDate
        ...
        // --- 共通ヘルパー: ポイント計算関数 (ポイント計算には pointCalcBaseDate ベースの年数を使用) ---
        ...
        // --- ポイント計算 (退職時 & 現在) - Uses pointCalcBaseDate ---
        
        // 現在時点(基準日 2025/09/30)のポイント（理論値）
        ...
        // 【修正】累積値が退職時理論最大値を超えないようキャップをかける（持ち越し分による超過を防ぐ）
        if (initialLosPointsInput + futureLosPoints > totalLosAtRetire) {
            futureLosPoints = Math.max(0, totalLosAtRetire - initialLosPointsInput);
        }
        if (initialRankPointsInput + futureRankPoints > totalRankAtRetire) {
            futureRankPoints = Math.max(0, totalRankAtRetire - initialRankPointsInput);
        }

        // Eval Points Calculation
        ...
        // 【修正】支給率係数の参照用勤続年数は「実際入社日」起算とする (6年ルール判定のため必須)
        ...
        // 【追加】平成26年4月3日以降入社かつ勤続6年未満の者は退職金支給なし
        ...
        const getEffectiveCoef = (yearsF: number, dateObj: Date) => {
            ...
            let table = coefSettings[typeKey]; // Default Current
            if (useTransition && transitionDate && dateObj > transitionDate) {
                 table = coefSettingsFuture[typeKey]; // Future
            }
        ...
        const calcPtAt = (yearsF: number, dateObj: Date, key: 'los'|'rank', rankK: string, mode: string) => {
            ...
            // Strictly check Transition Phase
            if (useTransition && transitionDate && dateObj > transitionDate) {
                ...
                // Explicitly use Current System Tables (Old or New depending on type)
                const cap = (key === 'rank' && useOldSystemTable) ? rankCapYearCurrent : 999;
        ...
        for (let y = baseYear; y <= maxLoopYear; y++) {
            ...
            if (y <= endYear) {
                ...
                const nextYearsFloat = calculatePeriodYears(pointCalcBaseDate, referenceDateY, getMode('los'));
                const nextYearsFloatForCoef = calculatePeriodYears(joinDate, referenceDateY, getMode('los'));
                ...
                // キャップ処理: 累積が退職時理論値を超えないように制限
                if (runningLos + incLos > totalLosAtRetire) {
                    incLos = Math.max(0, totalLosAtRetire - runningLos);
                }
                ...
                // キャップ処理
                if (runningRank + incRank > totalRankAtRetire) {
                    incRank = Math.max(0, totalRankAtRetire - runningRank);
                }
                ...
                const losAmountInc = Math.round(incLos * unitPrice * effectiveCoef);
                const rankAmountInc = Math.round(incRank * unitPrice * effectiveCoef);
                const evalAmountInc = Math.round(incEval * unitPrice * effectiveCoef);

                if ((useAdjustment || useUnify) && y === 2025) {
                    // 2025年度末(2026/3/31)時点での整合性強制
                    // B案（現行）の結果が渡されている場合、強制的にその値に合わせる
                    if (targetReserve2026 !== undefined) {
                        currentReserveAmount = targetReserve2026;
                        // runningLos等は逆算修正困難なためそのままだが、
                        // 以降の調整額計算（Gap計算）は reserveAmount2026Fixed をベースに行われるため金額の整合性は保たれる。
                    }
                ...
                if (y === endYear) {
                    // 特殊モード（調整/統一）を使用している場合は、強制的に計算結果を採用する
                    if (useAdjustment || useUnify) {
                        retirementAllowance = currentReserveAmount;
                    }
                    // 通常モード(Transition含む)の場合は、ループ計算の結果(currentReserveAmount)をそのまま使用し、
                    // 分析上のretirementAllowance(将来係数ベース)で上書きしない。
                    // これにより、移行前の年度(2025等)におけるPattern AとBの不整合を防ぐ。
                }
                ...
                yearlyDetails.push({ 
                    year: y, 
                    age: (y - birthDate.getFullYear()) + (birthDate.getMonth() < 3 ? 1 : 0), 
                    losPtInc: roundTo2(incLos), 
                    rankPtInc: roundTo2(incRank), 
                    evalPtInc: roundTo2(incEval), 
                    adjustmentPtInc: roundTo2(incAdj), 
                    amountInc: amountInc, 
                    losAmountInc: losAmountInc,
                    rankAmountInc: rankAmountInc,
                    evalAmountInc: evalAmountInc,
                    totalPt: roundTo2(runningLos + runningRank + runningEval + runningAdj), 
                    coef: effectiveCoef,
                    accLosPt: roundTo2(runningLos),
                    accRankPt: roundTo2(runningRank),
                    accEvalPt: roundTo2(runningEval),
                    accAdjustmentPt: roundTo2(runningAdj)
                });
            } else {
                 yearlyDetails.push({ 
                     year: y, 
        ...
        if (useUnify) {
             totalPoints = roundTo2(
                ...
                (initialRankPointsInput + futureRankPoints) + 
                (initialEvalPointsInput + futureEvalPoints) +
                yearlyDetails.filter(d => d.year >= 2026).reduce((s, d) => s + d.losPtInc + d.rankPtInc + d.evalPtInc, 0)
             );
        }
        ...
        return {
            ...
            totalPointsAtRetirement: totalPoints, 
            totalLosPoints: roundTo2(runningLos),
            totalRankPoints: roundTo2(runningRank),
            totalEvalPoints: roundTo2(runningEval),
            totalAdjustmentPoints: roundTo2(runningAdj),
            retirementAllowance: retirementAllowance > 0 ? retirementAllowance : 0, reserve2026, yearlyDetails, unitPrice
        };
    } catch (e) {
        logError("Calculation Error for row", e, { row });
        return null;
    }
};